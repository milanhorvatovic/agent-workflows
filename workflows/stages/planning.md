...
- **plan-approval** — after a passing validation. Transport per risk class ([overlays](../overlays.md)); blocking wherever planning runs. Outcomes: `accept` proceeds to the next stage in composition order — and where the accepted list places phases after this one, the run enters the next phase and this stage repeats before any of that, `run.phase` advancing with it (spec §10). In the `plan` workflow planning is the final stage, so completing the last phase's plan completes the run; completing phase 1's does not, that workflow's deliverable being the plan set its own list calls for, one per phase. What repeats is the planning it has: no implementation intervenes, so each later plan is written against the phase-1 list — the inputs `plan-create` declares reach no intervening phase's plan, and `{N}` being the only phase placeholder there is no declaration that would; `revise` returns to `plan-revise` — an explicit edge overriding the spec §7 default of returning to the step that produced the artifact, because a plan that passed validation on its first pass was produced by `plan-create`, and sending a human's direction there returns an existing plan to the step that writes one from scratch; revision is surgery on the plan the gate read, which is `plan-revise`'s whole contract, and it already declares every input this route needs — carrying the human's decisions and answers, recorded in the plan's **Gate direction** section before the outcome rather than in the gate record (spec §7), and carried from there into the revised plan, with the revision's feedback audit repeating it as working detail; `reject` splits on whether this gate has accepted a list, never on the phase number — phase 1 is re-entered after its own approval whenever implementation escalates a plan defect, and there the list is fixed and `run.phase` says which phase is executing, so the phase branch applies as it would to any later one. Before any acceptance it ends the run: the list is provisional, nothing names a next phase, and there is no `run.phase` to advance (spec §10). After one, it ends the current phase and the run moves to the next, or ends the run where that phase was the last.
