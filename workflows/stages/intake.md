...
- **Run-state bootstrap:** the run-state file is first written when `risk-route` proposes a class, because `run.risk` is required and `intake-approval` takes the `inbox` transport at R0–R2 ([overlays](../overlays.md)) — an asynchronous decision needs state to be resumable from, and a gate whose decision may be cleared by another driver cannot be the thing that creates it. The executor writes it, not `risk-route`: the step's own contract is to propose and never to write run state, and what the human accepts at the gate replaces the proposed class (spec §5.3), so the field holds a proposal only for as long as the proposal is what the run has. The clarifying question is the one gate that fires before run state exists, and its exchange is not resumable — deliberately so; it is one cheap question, and it is blocking in every class. Artifacts do not wait on run state either way: `{run}/brief.md` is written before it, direction included, so that gate still has somewhere to record what the human said (spec §7).
