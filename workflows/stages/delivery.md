...
The sequence (spec §9.4) declares the members in record order — reading order here, the gate's `revise` returning to `deliver-prepare` by spec §7's default with nothing ordered between them to invalidate:
