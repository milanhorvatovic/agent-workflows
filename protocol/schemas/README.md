...
The step, loop, and trigger schemas validate the value of a skill's `metadata.workflow` frontmatter key. Each is strict within its own structure but tolerates unknown *top-level* keys under `workflow` — siblings of the declared structures — per the 0.x degradation rules (spec §9.4); one block may carry `step`, `loop`, and `trigger` together. The run-state schema validates the whole state file and rejects unknown keys; that format is protocol-owned.
