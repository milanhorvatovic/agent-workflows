...
Two placeholders name a phase in those paths, and they are not interchangeable. `{N}` is **the phase the step is executing** — the one `run.phase` names ([10](#10-run-state)) — and resolves to exactly one artifact; it is what a stage a phase repeats uses, for the plan it is writing and the log it is filling. `{P}` is **each phase the run has completed**, and resolves to one artifact per such phase, none where the run has completed no phases and exactly one where it has completed one. A stage that runs once, after the last phase, has no phase of its own to name and reads the phases behind it with `{P}`; a step that would otherwise have to say in prose that its `{N}` means something other than the current phase is a step whose declaration wanted `{P}`.

A set-valued path does not change what `required` means, only what it counts: a required `{P}` input is missing, and blocks the step ([9.1](#91-step-and-handoff)), where any completed phase's artifact is absent, and an optional one is read for the phases that have it. A step MUST NOT declare `{P}` in its `output`: a step produces one artifact ([9.1](#91-step-and-handoff)), and a phase-set of outputs is a stage that repeats rather than a step that fans out.
