...
steps:
  ...
  - id: plan-approval # a gate carries its own entry; `blocked` while it waits, `done` once its outcome is recorded
    status: done
  - id: plan-revise # where the revise outcome routes; entered with the outcome, not after it (§7)
    status: pending
gates:
  - gate: plan-approval
