...
- `run.risk` and `risk_rationale` are absent before the intake gate accepts a class and present from then on, and they move together — the schema pairs them, so neither can be written without the other, at acceptance or at a later reclassification ([5.3](#53-override-and-reclassification)). Run state exists before them: it is created when the run starts, which is what gives a gate that fires during intake something to resume from.
