...
A gate decision MAY carry the human's direction — what they want changed, or the content of an answer to a question the gate asked. A gate record holds the outcome and never that content — its shape is closed to exactly the four fields the instrumentation requirement below names ([10](#10-run-state)), so this is enforced rather than asked for — and the direction is not an artifact either, so it has no place in an `inputs` declaration ([9.1](#91-step-and-handoff)): the step that acts on the direction MUST instead state in its instructions that it consumes it, and where the direction amends an artifact the run already holds, its content lands in that artifact rather than in the record. This binds on `accept` as much as on `revise`, since an `accept` confirming an amended artifact carries the amendment — but the failure it exists to prevent is the `revise` one: a `revise` whose direction is stated nowhere returns the run to a step that can only repeat itself.
