...
What may be imported at all is bounded first: the request ([8.7](#87-request)) MUST NOT be, whatever else a run adopts, since it has no producing step for the closure rule below to reach and a run executing another's plan is still a run somebody started. The run-state schema excludes that path from a lineage record for the same reason it constrains the rest of them.
