...
That rests on a resume returning to the gate rather than continuing past it, which the step statuses have to say. A step whose gate has fired and whose outcome is not yet recorded MUST NOT be marked `done`: it stays `blocked` ([10](#10-run-state)), and §8.5 resumes at the first step that is neither `done` nor `skipped`, so the resume lands on it and the gate is asked again. Marked `done` before its outcome exists, the step is skipped on resume and the run continues past a gate nobody answered — which would leave outcome-last ordering buying detection rather than recovery. A gate that fires before run state exists is outside that half: nothing can resume, so the exchange is re-run from the start rather than recovered, and the ordering buys no durability there — which is why such a gate is blocking, the exchange lasting no longer than the session holding it. What the reserved section buys at every gate, durable or not, is the separation above — the human's instruction reaches the step as an instruction rather than as content already folded into the artifact.
