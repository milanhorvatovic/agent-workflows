...
Direction that outlives the decision is therefore an artifact, and the executor MUST write it to `{run}/gate-direction.md` before recording the outcome. Resume is defined entirely by run state and requires no other memory ([8.5](#85-resume)), and an `inbox` transport is asynchronous by construction, so a decision may reach a driver other than the one that requested it: direction held only in the session that collected it survives neither. The file records which gate decided, when, and what the human said, and nothing else — it is a carrier, not a record of the run. The step that acts on the direction declares it like any other input ([9.1](#91-step-and-handoff)), reads what the human asked for, folds the content into whatever artifact it belongs in, and leaves the carrier spent. Writing the direction into the gated artifact instead would either pre-apply the change the human asked for or make their instruction indistinguishable from the content around it.

The ordering also decides which failure a crash produces, wherever the gate fires once run state exists: writing the carrier first re-asks a question whose answer is already on disk, where recording the outcome first loses the answer and keeps a decision nothing can act on. A gate that fires before run state exists is outside that half: nothing can resume, so the exchange is re-run from the start rather than recovered, and the carrier buys no durability there. What it buys at every gate, durable or not, is the separation above — the human's instruction reaches the step as an instruction rather than as content already folded into the artifact.

This binds on `accept` as much as on `revise`, since an `accept` confirming an amended artifact carries the amendment. A `revise` SHOULD carry direction. The protocol cannot judge whether a direction is adequate, so it is not a MUST, but a `revise` carrying none returns the run to a step whose inputs are unchanged, and nothing then distinguishes its next output from the one the gate has just declined.
