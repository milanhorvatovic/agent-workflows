...
The ordering also decides which failure a crash produces, wherever the gate fires once run state exists: writing the carrier first re-asks a question whose answer is already on disk, where recording the outcome first loses the answer and keeps a decision nothing can act on. A gate that fires before run state exists is outside that half — nothing can resume, so the exchange is re-run from the start rather than recovered — and the ordering still holds there, the carrier being where the answer survives either way.
