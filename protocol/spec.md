...
A gate decision MAY carry the human's direction — what they want changed, or the content of an answer to a question the gate asked. A gate record holds the outcome and never that content: its shape is closed to exactly the four fields the instrumentation requirement below names ([10](#10-run-state)), so this is enforced rather than asked for. The direction is not an artifact either, so it has no place in an `inputs` declaration ([9.1](#91-step-and-handoff)), and the step that acts on the direction MUST instead state in its instructions that it consumes it.

The executor MUST land the direction's content in an artifact of the run before recording the outcome. Resume is defined entirely by run state and requires no other memory ([8.5](#85-resume)), and an `inbox` transport is asynchronous by construction, so a decision may reach a driver other than the one that requested it: a direction held only in the session that collected it survives neither. Landing it first keeps run state and artifacts sufficient. The gated artifact is always a home for it — it is the artifact the human was deciding about.

The ordering also decides which failure a crash produces, wherever the gate fires once run state exists: landing first re-asks a question whose answer is already in the artifact, where recording first loses the answer and keeps a decision nothing can act on. A gate that fires before run state exists is outside that half — nothing can resume, so the exchange is re-run from the start rather than recovered — and the ordering still holds there, the artifact being where the answer survives either way.
