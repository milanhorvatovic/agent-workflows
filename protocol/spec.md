...
Re-entry resets more than the step it re-enters, and by the same write. A step run again — by a `revise` routing back to it, or by a loop iterating — invalidates what its output fed: the validator that must re-check it, the gate that must decide again. Those leave `done` when the step does, or the guarantee holds for exactly one step: the run resumes into the revision, and once that finishes finds nothing un-`done` after it and stops with the revised artifact never re-validated and never re-approved. So a `revise` is one write over four records — the outcome appended to `gates`, the reviser, the validator that must re-check it, and this gate itself all `pending`, ordered so the reviser is the first of them a resume finds. The gate's own entry is not `done` on a `revise`: it decided that the artifact must change, which is the decision to run this cycle again rather than one the run proceeds past.
