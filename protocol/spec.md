...
Each run owns a directory, `{artifacts}/runs/<run-id>/`, referred to in metadata as `{run}`. `{artifacts}` is the consuming project's artifact root: where it lives is project configuration, and the executor MUST resolve it identically for every step of a run. Concurrent runs and re-runs MUST NOT share a run directory. All step inputs and outputs are addressed relative to `{run}`, in the normalized form a lineage record also carries ([8.6](#86-import)) — anchored at `{run}/`, no `.` or `..` segments, no absolute form, and none of the spellings a platform aliases — which the step schema encodes. The executor joins a declared path to the run directory, so a shape left open is a declaration naming a file the run does not contain, or a platform's alias of one it does.
