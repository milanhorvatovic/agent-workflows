...
The executor MUST materialize the request at `{run}/request.md` when the run is created, before any step runs, and MUST add that path to the manifest ([8.2](#82-manifest)). What the file holds is whatever started the run, recorded as it arrived: the instruction's own text, or the reference a step will fetch through the executing harness's connections where the request is a pointer rather than prose. It has no template ([8.3](#83-templates)) — a request is not authored against a structure, it is what the run was given — and no producing step, so nothing scaffolds it and no step's completion is what puts it in the manifest. A run whose creation cannot land it MUST NOT be created, and refusing at creation is what keeps the id usable rather than spending it on a run that cannot start.

Whether a given step consumes the request is that step's contract, not a rule this section adds: [9.1](#91-step-and-handoff)'s completeness rule already obliges a step whose instructions restate the request to declare it, and a step declaring it required blocks where the run holds none. The workflows in this repository declare it on their entry step for that reason. An executor materializes what is declared, so a framework whose entry step reaches for the request without declaring it gets prose and no artifact — which is the defect §9.1 exists to make visible, in the framework rather than in the run.
