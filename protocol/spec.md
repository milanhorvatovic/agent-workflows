...
Two placeholders name a phase in those paths, and they are not interchangeable. `{N}` is **the phase the step is executing** — the one `run.phase` names ([10](#10-run-state)) — and resolves to exactly one artifact; it is what a stage a phase repeats uses, for the plan it is writing and the log it is filling. `{P}` is **each phase the run has already produced this artifact for**, and resolves to one path per such phase. What answers that is the manifest ([8.2](#82-manifest)), which records what the run has produced and only grows within it — so `{P}` needs no field of its own and no notion of a phase being finished, and it is right at every reader by construction: a step planning phase N sees the phases before it, because phase N's own artifact is what it is about to produce, and a stage running after the last phase sees them all. A stage that runs once, after the last phase, has no phase of its own to name and reads the phases behind it this way; a step that would otherwise have to say in prose that its `{N}` means something other than the current phase is a step whose declaration wanted `{P}`.

A set-valued path does not change what `required` means, only what it counts: a required `{P}` input blocks the step ([9.1](#91-step-and-handoff)) where the manifest records the artifact for no phase at all, and an optional one is read for whichever phases it records and is simply empty where that is none. It is the manifest of *this* run, which is what an optional `{P}` input cannot be satisfied from an earlier one: the cache in [8.4](#84-grounding-cache) hands over an artifact, and a phase set is a fact about the run that produced it. A step wanting both wants a different input. A step MUST NOT declare `{P}` in its `output`: a step produces one artifact ([9.1](#91-step-and-handoff)), and a phase-set of outputs is a stage that repeats rather than a step that fans out.
