...
- JSON Schemas (`protocol/schemas/`, draft 2020-12) for the four protocol structures — step/handoff, loop contract, trigger, run state — strict within each structure and tolerant of siblings per the `0.x` degradation rules; per-schema YAML fixtures in `examples/`: a commented minimal starter that validates and a deliberately-invalid counterpart that must fail.
