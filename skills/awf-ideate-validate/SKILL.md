...
What the matrix carries into the report goes in sections that already exist, which is why it gets none of its own: the weighting the brief implies is stated in the **Summary**, so a reader knows which criteria the verdict was weighed on before reading a single finding, and a score appears only inside the finding it drove. A full matrix is optional here, and an unexplained total that contradicts the findings is worse than no matrix at all — giving the scores a section of their own is exactly what would invite one.
