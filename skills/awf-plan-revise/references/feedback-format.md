...
| `direction[].action` | with `text` | `accept` or `escalate` — **not** the finding vocabulary. A finding is a claim the planner may disagree with; direction is the human's instruction, and declining it is not the planner's call. Where it cannot be followed — it would break the phase list a *previous* plan fixed — the answer is `escalate`, back to the human who gave it. Revising phase 1, the list is this plan's own content and direction to change it is ordinary revision, not an escalation: `{N}` is 1, `{run}/phase-1-plan.md` resolves to the plan under revision, and escalating there would return the human's instruction to the human who just gave it |
