...
| `direction[].action` | with `text` | `accept` or `escalate` — **not** the finding vocabulary. A finding is a claim the planner may disagree with; direction is the human's instruction, and declining it is not the planner's call. Where it cannot be followed — it would break a phase list that bounds work already planned or done — the answer is `escalate` the first time: the audit records that the planner did not follow it and why, and the plan carries the same thing as an **Open question** addressed to the human at `plan-approval`, which is what actually reaches them. This file is working material and reaches no gate, so an `escalate` recorded only here is a decision nobody sees. Direction that answers a question a previous iteration escalated is `accept` where `{N}` is 1, the list being in the plan under revision: the cost was stated, the human decided with it in view, and re-escalating would put the same question to the person who has just answered it and spend the loop on it. Where `{N}` is greater than 1 it stays `escalate` with the authorization recorded in the reason, because the list lives in `phase-1-plan.md` and this step's one declared output is not that artifact — the authorization is real and the run needs re-planning to act on it, which no outcome of this gate performs: a `revise` returns to the same wall and a `reject` ends the current phase and moves to the next, carrying on under the decomposition being replaced. Acting on it means a new run, recorded here and stated in the plan so the human sees what their answer actually costs. Before `plan-approval` fixes the list it is `accept` where the list is the only obstacle: the list is still provisional, `{run}/phase-1-plan.md` resolves to the plan under revision, and escalating would hand the instruction back with nothing the first ask did not already have. Direction blocked by anything else — infeasible, contradicting the brief, departing from a project standard — is `escalate` before approval as after, since `accept` is the only alternative the vocabulary offers and it would buy a plan that fails validation rather than a question the human can answer |
