...
- `{run}/brief.md` (required) — the confirmed brief, and the only authority on intent. Its goal, constraints, acceptance criteria, and out-of-scope boundaries are what is being classified, and the assumptions it records are part of the ambiguity reading.
...
- `{run}/gate-direction.md` (optional) — the answer to a `clarifying-question` this step raised, which an `accept` there resumes at classification to consume rather than returning to `brief-confirm`. A gate record carries the outcome and never the answer's content, so the executor writes it here before recording the outcome (spec §7). This gate fires before run state exists, so nothing resumes and the carrier buys no durability here; what it buys is that the answer reaches this step as something to apply rather than as prose already folded into the brief by whoever collected it. Optional because most runs reach classification with no question asked. Never satisfied from an earlier run: §8.4's cache does not reach a record of this run's decision.
...
The answer comes back to this step rather than to `brief-confirm`, since an `accept` there resumes at classification whichever step raised the question (the intake stage's gate contract). Read it from `{run}/gate-direction.md`, fold what it changes into the brief section it changes rather than leaving it only in the Routing note that asked for it, and classify against the brief as amended. The carrier is spent once its content is in the brief — a run reaching the intake gate with an unconsumed direction is a question that was answered and never applied.
