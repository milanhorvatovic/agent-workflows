...
- `{run}/brief.md` (required) — the confirmed brief, and the only authority on intent. Where a `clarifying-question` this step raised was answered, what arrives is the brief `brief-confirm` amended with that answer, not the answer itself: applying it is that step's work and reading the result is this one's. Its goal, constraints, acceptance criteria, and out-of-scope boundaries are what is being classified, and the assumptions it records are part of the ambiguity reading.
...
The answer does not come back to this step to be applied. A clarifying answer can change the goal, a constraint, or an acceptance criterion, and restating the brief is `awf-brief-confirm`'s work, not this step's — so an `accept` on a question raised here returns to `brief-confirm` to fold the answer in, and composition carries the run back to classification after it (the intake stage's gate contract). What arrives here is the amended brief: classify it, and where the answer resolved what the Routing note said could not be pinned down, propose the class it now supports.
