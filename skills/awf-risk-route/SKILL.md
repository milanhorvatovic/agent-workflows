...
The answer comes back to this step rather than to `brief-confirm`, since an `accept` there resumes at classification whichever step raised the question (the intake stage's gate contract). Consume it and classify against the brief as amended: the gate record carries the outcome and never the answer's content (spec §7), so where the answer changes what the brief says, that content belongs in the section it changes and not only in the Routing note that asked for it.
