...
[Ambiguities the planner could not confidently resolve, each specific and answerable, with proposed alternatives. Escalations land here too: a revision that cannot follow the human's direction or an accepted finding without breaking a phase list approval has already fixed states what the change would require, since this is the section the gate reads and the feedback audit is not an artifact it is given. "None" when everything is decided.]
