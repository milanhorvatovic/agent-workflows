...
- `{run}/brief.md` (optional) — this step's own prior output, which a re-entry revises rather than replaces, and where a gate's direction is waiting: whatever the human said at the `clarifying-question` or `intake-approval` gate is recorded in the brief's **Gate direction** section before the outcome is (spec §7), so no separate input carries it. Read that section first, fold each item into the sections it is about, and leave it empty — it holds an instruction to this step, never content of the brief. Durability is what the section buys at `intake-approval`, whose decision is asynchronous at R0–R2 and may be cleared by a driver other than the one that requested it; at `clarifying-question` it buys none — that gate fires before run state exists and is blocking in every class, so nothing resumes and the exchange lasts no longer than the session holding it. What it buys at both is that the instruction arrives *as* one rather than pre-applied into prose indistinguishable from what the brief already said. Optional on availability rather than caution: on a first run the artifact cannot precede the step that writes it, and its absence is what says this is a first run. Optional puts it in reach of the spec §8.4 cache, so the freshness check §9.1 requires is stated here as the one that applies: it is never satisfied from an earlier run, being this run's own restatement of this run's request.
