...
Additional checks for a plan whose Phase list section fixes the run's phase list (a phase-1 plan of multi-phase work). Loaded on top of the standard method, which still applies to the phase-1 steps themselves. The phase list is fixed for the run once this plan passes — an error that survives here surfaces phases later, as cross-phase escalation.
