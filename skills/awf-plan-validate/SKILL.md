...
- `{run}/phase-1-plan.md` (required) — the phase list: what says which phase owns which requirement. What decides whether it is provisional or fixed is approval, never the phase number: before `plan-approval` accepts it the list is provisional and this step is one of the two things that can still re-cut it, and after that acceptance it binds — at every later phase, and equally at phase 1, since this loop is re-entered after approval whenever implementation escalates a plan defect or another phase cascades into it. The plan's changelog is where that shows: a row whose trigger is implementation feedback or a cascade is one no plan can carry before the gate accepted it, and neither trigger un-happens, so the list is fixed from the first such row onward. Reading `{N}` instead would hand a fixed list back to a revision that `awf-plan-revise` will refuse to make, and the loop would spend its cap on the disagreement. Without it this step cannot tell work another phase owns from work this plan dropped, so a later phase's plan could pass with its coverage unchecked. Required rather than optional because it always exists wherever this step runs: validating phase 1 or a single-phase run, `{N}` is 1 and this resolves to the plan already under validation; validating a later phase, phase 1 was planned before that phase could exist. Never satisfied from another run — spec §8.4's cache does not reach a decomposition made for a different run — so check the plan's `Run` header and stop rather than validate coverage against the wrong list.
- `{run}/phase-{P}-impl-validation.md` (optional) — what the completed phases bind for the one under validation, from the **Cross-phase impact** section each of them fills. This is what lets the plan be checked against those bindings: a plan that contradicts a rollout order, or drops a migration step an earlier phase established, is defective in a way the codebase cannot show, the constraint being about sequence rather than about what exists. A finding here names the binding and the phase that set it. `{P}` rather than `{N}` for the reason spec §8.1 gives — a step never reads its own phase through it, which matters here because implementation feedback re-entering planning leaves this phase's own report in the manifest, and that report binds the phases after this one rather than this plan. Optional because phase 1 has nothing behind it, and owed no freshness check: the token resolves against this run's manifest, so §8.4's cache cannot reach it.
