...
Append one section, **Coverage**, above the findings: the requirement-by-requirement mapping from the brief's requirements and acceptance criteria to the plan step or steps addressing each, marked covered, partially covered, uncovered, or — only where the paragraph below applies — not assessable. In a phase-1 plan of multi-phase work a row may point instead to the phase-list entry that owns the requirement — that plan carries the list, and the phase-decomposition checks walk the brief across all of it, so demanding a phase-1 step for work a later phase owns would either read as uncovered or push the plan to invent steps it has no business fixing yet. A later phase's plan marks the list fixed elsewhere and does not carry it, and this step receives only that plan and the brief — so it can map the requirements the plan addresses, but cannot confirm which phase the fixed list assigned any requirement to. A requirement that plan does not address is therefore **not assessable** rather than uncovered — it may belong to another phase, and calling it uncovered asserts a fact this step cannot establish, while calling it covered hides one. Use that state only here, say in the row that the phase list was unavailable, and raise a finding for every row that carries it: not assessable is an open question about coverage, never a pass, and a later phase's plan must not clear this step on rows nobody could check. Each finding names the requirement and the artifact that would settle it — the fixed phase list, which this step does not receive. Inferring a boundary from the plan under validation would only check the plan against itself; whether every requirement reaches some phase is the phase-1 validation's check, where the list is present. The completeness walk above produces it, and it belongs in the artifact for the same reason `validate-tickets` gives its own coverage table — the check's result is not a boolean. The core **Completeness** and **Scope** rows record whether coverage holds; this records how that was established, and a partially covered requirement is the finding most easily lost when there is nowhere to write down that it is only half addressed.
