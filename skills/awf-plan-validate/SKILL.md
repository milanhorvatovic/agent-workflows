...
Exactly one verdict — PASS, PASS_WITH_CONDITIONS, or FAIL (spec §3.3) — with unresolved critical findings or blocking questions forcing FAIL. A question blocks when the plan cannot be implemented until it is answered. One the plan records for the human at `plan-approval` — a direction the revision could not follow, a boundary only they can move — does not: the plan is implementable as written and what is open is whether they want it changed, so failing it would loop the plan back to the planner who already said the answer is not theirs. The verdict is consumed by the planning loop's exit criteria and routes the plan to the plan-approval gate or back to `plan-revise`.
