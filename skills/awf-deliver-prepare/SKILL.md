...
- `{run}/phase-{N}-impl-validation.md` (optional) — the implementation verdicts, absent at R1 where the validator is skipped.
...
Report verification as evidence, not assertion: which machine checks ran and their result, which tests cover the new behavior, and what each validator concluded, including conditions attached to a PASS_WITH_CONDITIONS and any finding left open by an accepted verdict. Where a validation artifact is absent because its step was skipped for the risk class, say so — an absent verdict is a fact about the run, not a gap to paper over.
...
At R1 the artifact is a minimal change note per the risk-class overlays — summary, what changed, the verification that ran, and the change description. The template's *(R2+)* sections are omitted at that class rather than filled with placeholders.
