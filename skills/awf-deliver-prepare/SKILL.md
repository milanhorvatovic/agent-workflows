...
Close on what the change carries forward once it is live: the risks worth naming with the signals that would show them and the path back — the revert, the migrations that need reversing, the flags to flip — and the follow-ups this run deliberately left, which are the findings-for-planning the logs raised, the findings left open under an accepted verdict, and the gaps the logs record, each with its id and where it is recorded. Both come from the sources this step already reads; neither is an invitation to invent work the run never considered, and "None" is a legitimate answer to either.
